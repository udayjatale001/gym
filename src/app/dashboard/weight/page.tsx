  ...
  return (
    <div className="p-4 space-y-10 pb-32 min-h-svh animate-in fade-in slide-in-from-bottom-4 duration-500 no-scrollbar">
      <div className="flex items-center justify-between pt-6 px-1">
        <div className="flex items-center gap-4">
          <div className="h-14 w-14 md:h-16 md:w-16 rounded-[1.5rem] md:rounded-[1.8rem] bg-primary flex items-center justify-center text-primary-foreground shadow-2xl shadow-primary/30 border-b-4 border-black/20">
            <Scale className="h-7 w-7 md:h-9 md:w-9" />
          </div>
          <div className="space-y-1">
            <div className="flex items-center gap-3">
              <h2 className="text-2xl md:text-3xl font-black text-primary uppercase tracking-tighter italic leading-none">BODY MASS</h2>
      ...
      <Card className="border-none shadow-2xl rounded-[3rem] overflow-hidden bg-card relative active:scale-[0.99] transition-all">
        ...
        <CardContent className="p-8 md:p-10 space-y-10">
          <div className="space-y-4">
            <p className="text-[10px] md:text-[11px] font-black text-muted-foreground uppercase tracking-[0.4em] px-2 opacity-60">CURRENT MASS (KG)</p>
            <div className="relative">
              <Input 
                type="text" 
                inputMode="decimal" 
                placeholder="00.0" 
                className="h-24 md:h-28 text-5xl md:text-6xl font-black border-4 border-muted rounded-[2rem] focus-visible:ring-primary shadow-inner pl-8 pr-20 bg-muted/5 placeholder:text-muted-foreground/10 text-primary text-base md:text-6xl" 
                value={newWeight} 
                onChange={(e) => setNewWeight(e.target.value.replace(/[^0-9.]/g, ''))} 
              />
              <span className="absolute right-8 md:right-10 top-1/2 -translate-y-1/2 text-2xl md:text-3xl font-black text-primary/30 italic">KG</span>
            </div>
          </div>
          <Button onClick={handleAddWeight} className="w-full h-20 md:h-24 text-xl md:text-2xl font-black uppercase italic rounded-[2rem] shadow-2xl active:scale-95 transition-all bg-primary" disabled={isSubmitting || !newWeight.trim()}>
            {isSubmitting ? <Loader2 className="h-7 w-7 animate-spin" /> : <span className="flex items-center gap-4"><Plus className="h-7 w-7 md:h-8 md:w-8" /> CONFIRM ENTRY</span>}
          </Button>
        </CardContent>
      </Card>
        ...
        <div className="flex flex-col gap-4">
            ...
            <Card key={log.id} className="border-none shadow-xl active:scale-[0.98] rounded-[2.5rem] bg-card border-l-8 border-l-primary overflow-hidden transition-all hover:bg-muted/30">
              <CardContent className="p-6 md:p-8 flex items-center justify-between gap-4">
                <div className="flex items-center gap-4 md:gap-6">
                  <div className="h-14 w-14 md:h-16 md:w-16 rounded-[1.25rem] md:rounded-[1.5rem] bg-primary/10 flex items-center justify-center text-primary shadow-inner shrink-0"><Scale className="h-7 w-7 md:h-8 md:w-8" /></div>
                  <div className="min-w-0">
                    <p className="text-3xl md:text-4xl font-black text-primary italic leading-none truncate">{log.weight}<span className="text-[10px] md:text-[12px] uppercase not-italic opacity-30 ml-2 font-bold tracking-widest">KG</span></p>
                    <p className="text-[9px] md:text-[10px] text-muted-foreground font-black uppercase mt-2 opacity-50 flex items-center gap-2 tracking-widest truncate"><Calendar className="h-3 w-3" />{format(new Date(log.timestamp), 'MMM dd • h:mm a')}</p>
                  </div>
                </div>
          ...
          {logs.length === 0 && (
            <div className="text-center py-20 bg-muted/10 rounded-[3rem] border-4 border-dashed border-border/50 flex flex-col items-center gap-4">
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground/40 italic">Awaiting discipline...</p>
            </div>
          )}
        </div>
      </div>
      ...
      <Sheet open={isProgressOpen} onOpenChange={setIsProgressOpen}>
        <SheetContent side="bottom" className="rounded-t-[3rem] md:rounded-t-[4rem] h-[85svh] border-none p-0 overflow-hidden bg-background">
          <div className="h-full overflow-y-auto no-scrollbar p-8 md:p-10 space-y-10 md:space-y-12 pb-32">
            <SheetHeader><SheetTitle className="text-3xl md:text-4xl font-black uppercase italic tracking-tighter text-primary text-center">GOAL METRICS</SheetTitle></SheetHeader>
            <div className="space-y-8 md:space-y-10">
              <div className="grid grid-cols-2 gap-4 md:gap-6">
                <Card className="bg-card border-none rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-8 text-center shadow-2xl">
                  ...
                  <p className="text-4xl md:text-5xl font-black italic text-primary">{currentWeight || "--"}<span className="text-xs italic opacity-40 ml-1">KG</span></p>
                </Card>
                <Card className="bg-card border-none rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-8 text-center shadow-2xl">
                  ...
                  <div className="flex items-center justify-center gap-1">
                    <Input 
                      ...
                      className="text-center font-black text-2xl md:text-3xl h-10 border-none bg-transparent focus-visible:ring-0 p-0 text-accent placeholder:text-accent/20 shadow-none w-20 md:w-24 text-base md:text-3xl" 
                    />
                    <span className="text-xs font-black italic text-accent opacity-40">KG</span>
                  </div>
                </Card>
              </div>
              <Card className="p-8 md:p-12 rounded-[2.5rem] md:rounded-[3.5rem] shadow-2xl bg-card border-none space-y-8 md:space-y-10 relative overflow-hidden">
                 <div className="flex justify-between items-end mb-4"><h3 className="text-[10px] md:text-[11px] font-black uppercase tracking-[0.3em] opacity-60 flex items-center gap-2"><TrendingUp className="h-4 w-4 text-primary" /> DISCIPLINE</h3><span className="text-2xl md:text-3xl font-black text-primary italic leading-none">{Math.round(progressValue)}%</span></div>
                 <Progress value={progressValue} className="h-6 md:h-8 bg-muted rounded-full shadow-inner" />
                 {goalWeight > 0 && (
                  <div className="text-center py-6 md:py-8 bg-muted/10 rounded-[1.5rem] md:rounded-[2rem] border-4 border-dashed border-border/50">
                    <p className="text-[9px] font-black uppercase opacity-50 tracking-[0.4em] mb-2">REMAINING GAP</p>
                    <p className="text-4xl md:text-5xl font-black italic tracking-tighter">{Math.abs(currentWeight - goalWeight).toFixed(1)} <span className="text-sm italic opacity-30 tracking-normal">KG</span></p>
                  </div>
                 )}
              </Card>
              <Button className="w-full h-20 md:h-24 rounded-[1.5rem] md:rounded-[2rem] font-black uppercase italic text-xl md:text-2xl shadow-2xl bg-primary active:scale-95" onClick={handleSaveTarget}>SAVE DISCIPLINE</Button>
            </div>
          </div>
        </SheetContent>
      </Sheet>
